...
def adicionarContato() -> None:
    ...
    contato = {"nome": nome, "telefone": telefone, "categoria": categoria}
    agenda.append(contato)
    messagebox.showinfo("Adicionar!",f"Nome: {nome}" f"Telefone:{telefone}")
    limparCampos()
    atualizarTabela()
    print(agenda)
...
def editarContato() -> None:
    ...
    if opcao:
        agenda[index]= { "nome": txt_nome.get(), "telefone": txt_telefone.get(), "categoria": cb_categorias.get()}
...
def excluirContato() -> None:
    opcao = messagebox.askyesno("Tem ceteza?", "Deseja deletar o contato?")
    if opcao:
